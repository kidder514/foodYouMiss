	...
	generateGallery(imgs) {
		...
		if(imgs.length > 4){
			...
			for(let i = 0; i < imgs.length; i++){
				if( imgs[i] != null)
					imgNodes += <div className="nine-layout"><img src={imgs[i]} /></div>;
			}
		}else if(imgs.length >= 2 && imgs.length <= 4){
			for(let i = 0; i < imgs.length; i++){
				if( imgs[i] != null)
					imgNodes += <div className="four-layout"><img src={imgs[i]} /></div>;
			}
		}else{
			for(let i = 0; i < imgs.length; i++){
				if( imgs[i] != null)
					imgNodes += <div className="one-layout"><img src={imgs[i]} /></div>;
			}
		}
	    ...
	    return (
	    	<div className="single-post">
	            <div className="left-section">
	            	...
	            	<Link to="/author/:{post.authorId}"><p>{post.authorName}</p></Link>
	            </div>
	            <div className="right-section">
	            	...
	            	<Link to="/map/:{post.coordinate}" className="post-location" >{post.postLocation}</Link>
	            	<span>:expand</span>
